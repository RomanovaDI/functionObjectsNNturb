...
Foam::functionObjects::fieldAverageTurbItem::fieldAverageTurbItem(Istream& is)
:
    active_(false),
    fieldName_("unknown"),
    mean_(false),
...
)
{
    ...
    if (faItem.window_ > 0)
    {
        ...
        if (faItem.windowType_ != fieldAverageTurbItem::windowType::NONE)
        {
            if
            (
                faItem.base_ == fieldAverageTurbItem::baseType::ITER
             && label(faItem.window_) < 1
            )
            {
                FatalIOErrorInFunction(dictEntry)
                    << "Window must be 1 or more for base type "
                    << faItem.baseTypeNames_[fieldAverageTurbItem::baseType::ITER]
                    << exit(FatalIOError);
            }

            faItem.windowName_ = dict.getOrDefault<word>("windowName", "");

            if (faItem.windowType_ == fieldAverageTurbItem::windowType::EXACT)
            {
    ...
    faItem.meanFieldName_ = faItem.fieldName_ + fieldAverageTurbItem::EXT_MEAN;
    faItem.prime2MeanFieldName_ =
        faItem.fieldName_ + fieldAverageTurbItem::EXT_PRIME2MEAN;
