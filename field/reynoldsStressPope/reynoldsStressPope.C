...
/*
    // T3 = s & s - I * tr(s & s) / 3
    {
        // Calculate trace of s & s
        volScalarField tr_ss = tr(ss);
        tr_ss.dimensions().reset(strainSqDim);
        
        // Create temporary field with correct dimensions
        volTensorField T3temp(ss);
        forAll(mesh_.C(), cellI)
        {
            T3temp[cellI] -= (tr_ss[cellI] / 3.0) * I;
        }
        T3_ = T3temp;
        T3_.dimensions().reset(strainSqDim);
    }
    ...
    // T4 = r & r - I * tr(r & r) / 3
    {
        // Calculate trace of r & r
        volScalarField tr_rr = tr(rr);
        tr_rr.dimensions().reset(strainSqDim);
        
        // Create temporary field with correct dimensions
        volTensorField T4temp(rr);
        forAll(mesh_.C(), cellI)
        {
            T4temp[cellI] -= (tr_rr[cellI] / 3.0) * I;
        }
        T4_ = T4temp;
        T4_.dimensions().reset(strainSqDim);
    }
    ...
    // T5 = r & s & s - s & s & r
    {
        volTensorField rss(r_ & ss);
        rss.dimensions().reset(strainCubeDim);
        
        volTensorField ssr(ss & r_);
        ssr.dimensions().reset(strainCubeDim);
    ...
    // T6 = r & r & s + s & r & r - I * 2 * tr(s & r & r) / 3
    {
        volTensorField rrs(rr & s_);
        rrs.dimensions().reset(strainCubeDim);
        
        volTensorField srr(s_ & rr);
        srr.dimensions().reset(strainCubeDim);
        
        // Calculate trace of s & r & r
        volTensorField srr_temp(s_ & rr);
        srr_temp.dimensions().reset(strainCubeDim);
        volScalarField tr_srr = tr(srr_temp);
        tr_srr.dimensions().reset(strainCubeDim);
        
        // Create temporary field
        volTensorField T6temp(rrs + srr);
        forAll(mesh_.C(), cellI)
        {
            T6temp[cellI] -= (2.0 * tr_srr[cellI] / 3.0) * I;
        }
        T6_ = T6temp;
        T6_.dimensions().reset(strainCubeDim);
    }
    ...
    // T7 = r & s & r & r - r & r & s & r
    {
        volTensorField rsrr(rs & rr);
        rsrr.dimensions().reset(strainQuartDim);
        
        volTensorField rrsr(rr & sr);
        rrsr.dimensions().reset(strainQuartDim);
    ...
    // T8 = s & r & s & s - s & s & r & s
    {
        volTensorField srss(sr & ss);
        srss.dimensions().reset(strainQuartDim);
        
        volTensorField ssrs(ss & rs);
        ssrs.dimensions().reset(strainQuartDim);
    ...
    // T9 = r & r & s & s + s & s & r & r - I * 2 * tr(s & s & r & r) / 3
    {
        volTensorField rrss(rr & ss);
        rrss.dimensions().reset(strainQuartDim);
        
        volTensorField ssrr(ss & rr);
        ssrr.dimensions().reset(strainQuartDim);
        
        // Calculate trace of s & s & r & r
        volTensorField ssrr_temp(ss & rr);
        ssrr_temp.dimensions().reset(strainQuartDim);
        volScalarField tr_ssrr = tr(ssrr_temp);
        tr_ssrr.dimensions().reset(strainQuartDim);
        
        // Create temporary field
        volTensorField T9temp(rrss + ssrr);
        forAll(mesh_.C(), cellI)
        {
            T9temp[cellI] -= (2.0 * tr_ssrr[cellI] / 3.0) * I;
        }
        T9_ = T9temp;
        T9_.dimensions().reset(strainQuartDim);
    }
    ...
    // T10 = r & s & s & r & r - r & r & s & s & r
    {
        volTensorField rssrr(rs & ss & rr);
        rssrr.dimensions().reset(strainQuintDim);
        
        volTensorField rrssr(rr & ss & r_);
        rrssr.dimensions().reset(strainQuintDim);
