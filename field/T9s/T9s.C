...
{
    defineTypeNameAndDebug(T9s, 0);
    addToRunTimeSelectionTable(functionObject, T9s, dictionary);
}
}
...
bool Foam::functionObjects::T9s::calc()
{
    if (foundObject<volVectorField>(fieldName_))
    {
        ...
        (
            resultName_,
            symm((r & r & s & s) + (s & s & r & r) - tensor::one * (2 * tr(s & s & r & r) / 3))
        );
    }
